...
containerID := $(shell docker ps -a -q --filter ancestor=timnuwa/kafkademo:latest)
...
#===================  docker  ===========================
docker-build: build
	docker build -t timnuwa/kafkademo:latest .


docker-run: docker-build
	docker run -p 8080:8080 timnuwa/kafkademo:latest
...
docker-push: docker-login docker-build
	docker push timnuwa/kafkademo:latest

docker-bash:
	docker exec -ti timnuwa/kafkademo:latest /bin/sh
...
k8s-persistence-create:
	...
	echo "kubectl run -it --rm --image=mysql:5.6 --restart=Never mysql-client -- mysql -h kafkademo-db -ppassword -e 'create database db_example;'"
...
k8s-info:
	...
	kubectl get pods
	echo "You can also check DB via:\n kubectl run -it --rm --image=mysql:5.6 --restart=Never mysql-client -- mysql -h kafkademo-db -ppassword "
